...
class ProtocolInterface implements SourceInterface{
	...
	public function __construct(PacketLink $plugin, $server, Translator $translator, $threshold){
		$this->plugin = $plugin;
		$this->server = $server;
	...
	protected function handlePacket(DesktopPlayer $player, $payload){
		...
		$status = $player->packetLink_getStatus();
		...
		if($status === 1){
			...
			if($pid === 0x00){
				...
				$pk->read($payload, $offset);
				$player->packetLink_handleAuthentication($this->plugin, $pk->name, $this->plugin->isOnlineMode());
			}elseif($pid === 0x01 and $this->plugin->isOnlineMode()){
				echo "EncryptionResponse\n";
				$pk = new EncryptionResponsePacket();
				$pk->read($payload, $offset);
				$player->packetLink_processAuthentication($this->plugin, $pk);
			}else{
