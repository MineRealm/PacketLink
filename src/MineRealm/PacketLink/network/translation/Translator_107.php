...
use MineRealm\PacketLink\PacketLink;
...
use MineRealm\PacketLink\network\protocol\Play\Server\TabCompletePacket;
...
use MineRealm\PacketLink\utils\ConvertUtils;
	...
	public function serverToInterface(DesktopPlayer $player, DataPacket $packet){
		switch($packet->pid()){
			case ProtocolInfo::DISCONNECT_PACKET:
				if($player->packetLink_getStatus() === 0){
					$pk = new LoginDisconnectPacket();
					$pk->reason = PacketLink::toJSON($packet->message === "" ? "You have been disconnected." : $packet->message);
				}else{
			...
			case ProtocolInfo::TEXT_PACKET:
				if($packet->message === "chat.type.achievement"){
					...
					$pk = new ChatPacket();
					$pk->message = PacketLink::toJSON($packet->message, $packet->source, $packet->type, $packet->parameters);
					switch($packet->type){
			...
			case ProtocolInfo::START_GAME_PACKET:
				...
				$pk->dimension = $player->packetLink_getDimensionPEToPC($pk->dimension);
			...
			case ProtocolInfo::ADD_ITEM_ENTITY_PACKET://Bug
			...
			case ProtocolInfo::SET_ENTITY_DATA_PACKET:
				...
				if($player->getSetting("BossBar") !== false){
					if($packet->eid === $player->getSetting("BossBar")[0]){
						...
						$pk->title = PacketLink::toJSON($title);
			...
			case ProtocolInfo::CONTAINER_OPEN_PACKET:
				...
				$pk->windowTitle = PacketLink::toJSON($title);
					...
					case 0://Add
						...
						foreach($packet->entries as $entry){
							...
							if($packetplayer instanceof DesktopPlayer){
								$peroperties = $packetplayer->packetLink_getPeroperties();
							}else{
								//TODO: Skin Problem
								$value = [//Dummy Data
									"timestamp" => 0,
