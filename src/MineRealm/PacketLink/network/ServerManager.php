...
class ServerManager{
	...
	public function __construct(ServerThread $thread, $port, $interface, $description = "", $favicon = null){
		...
		if(!$this->socket){
			$this->logger->critical("[PacketLink] **** FAILED TO BIND TO " . $interface . ":" . $port . "!", true, true, 0);
			$this->logger->critical("[PacketLink] Perhaps a server is already running on that port?", true, true, 0);
			exit(1);
		}
